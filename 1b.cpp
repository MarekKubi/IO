...
class KubickiPizzaBuilder : public PizzaBuilder {
public:
	void buildDough() override
	{
		pizza.setDough("Cheese Dough");
	}
	void buildSauce() override
	{
	...
	void buildTopping() override
	{
		pizza.setTopping("Cheese and Cheese");
	}
	Pizza getPizza() const override { return pizza; }
private:
	Pizza pizza;
...
int main()
{
	...
	KubickiPizzaBuilder KubickiBuilder;
	cook.makePizza(KubickiBuilder);
	Pizza KubickiPizza = KubickiBuilder.getPizza();
	KubickiPizza.displayPizza();
	return 0;
}